sap.ui.define([
    ...
    return {
        ...
        //Compare Function to sort the employee names array into alphabetical order
        ...
        createNewEntryAndDialog: function (oEvent, oController) {
            ...
            if (selectedKey === 'PDdevItems' || selectedKey === 'Action') {
                ...
                if (feedbackArray.length === 0) {
                    MessageToast.show("No Feedback Report Created!");
                } else {
            ...
            //if(selectedKey !== 'Reviews' && feedbackModel.length !== 0){
        ...
        readFeedBackToReview: function (oController, feedBack) {
            ...
            oController.getView().getModel("oDataFeedback").read(path, {
                success: function (data, oResponse) {
                    console.log("Feedback Entry found in DB");
                    settingsModel.setProperty("/RoleAttributes", data);

                },
                error: function (error) {
                    console.log("Error! Could not Read feedback Entry from DB!");
                }
            });
        },
        ...
        loadNotes: function (oController) {
            ...
            oController.getPDPlanFromBackend("FilterOutArchived", "/DevelopmentItemsList", "PDplans(PDplanID=" + selectedPDP.PDplanID + ")",
                ...
                selectedItem, {
                    success: function (data, oResponse) {
                        ...
                        if (entitySet === "PDplans") {
                            oController.getPDPlanFromBackend("FilterOutArchived", "/PDplansList", "PDplans", null);

                            //A Dev Item was updated, refresh the list
                        } else if (entitySet === "DevelopmentItemsList") {
                            debugger
                            oController.getPDPlanFromBackend("ShowAll", "/DevelopmentItemsList", "PDplans(PDplanID=" + parentPDplan.PDplanID +
                                ")",
                                "to_DevItem/to_goals,to_DevItem/to_notes");
